...
export default function Settings() {
  ...
  // Compute total usage across all providers
  ...
  return (
    <div className="min-h-screen pt-20 pb-20 px-4 md:pl-24 md:pr-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-surface-container-low border border-outline-variant/10 rounded-xl p-5">
              <h3 className="font-semibold text-on-surface mb-4 flex items-center gap-2 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-surface-container-high rounded-lg p-3 text-center">
                  <p className="text-2xl font-bold font-headline text-on-surface">{configuredCount}</p>
                  <p className="text-xs text-on-surface-variant mt-0.5">已設定</p>
                </div>
                <div className="bg-surface-container-high rounded-lg p-3 text-center">
                  <p className="text-2xl font-bold font-headline text-tertiary">{totalUsage}</p>
                  <p className="text-xs text-on-surface-variant mt-0.5">今日呼叫</p>
                </div>
              </div>
                ...
                {AI_PROVIDERS.filter(p => !!keys[p.id]).map(p => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${getStatusColor(p.id)}`} />
            ...
            <div className="bg-surface-container-low border border-outline-variant/10 rounded-xl p-5">
              ...
              <div className="space-y-2 text-xs text-on-surface-variant leading-relaxed">
                <div className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-tertiary text-[14px] mt-0.5 flex-shrink-0">lock</span>
                  <span>API Key 以 <strong className="text-on-surface">AES-256</strong> 加密儲存於本機 LocalStorage</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-tertiary text-[14px] mt-0.5 flex-shrink-0">visibility_off</span>
                  <span>Key 不會傳送至我們的伺服器，也不暴露於前端網路請求</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-tertiary text-[14px] mt-0.5 flex-shrink-0">swap_horiz</span>
                  <span>API 呼叫透過後端 Proxy 轉發，安全無虞</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface-container-low border border-outline-variant/10 rounded-xl p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { name: 'Replicate', url: 'replicate.com/account/api-tokens', icon: 'hub' },
                  { name: 'Stability AI', url: 'platform.stability.ai/account/keys', icon: 'image' },
                ].map(item => (
                  <div key={item.name} className="flex items-start gap-2">
                    <span className="material-symbols-outlined text-on-surface-variant text-[14px] mt-0.5 flex-shrink-0">{item.icon}</span>
                    <div>
                      <p className="text-xs font-semibold text-on-surface">{item.name}</p>
                      <p className="text-[11px] text-primary/80 break-all">{item.url}</p>
                    </div>
                  </div>
                ))}
              </div>
