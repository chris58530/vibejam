...
### 2. 建立 `.env` 檔案
```
DATABASE_URL=postgresql://user:password@localhost:5432/vibejam
...
# 或用 Docker
docker run --name vibejam-db -e POSTGRES_DB=vibejam -e POSTGRES_PASSWORD=password -p 5432:5432 -d postgres:15
```
...
git remote add origin https://github.com/your-username/vibejam.git
...
- 連接你的 vibejam GitHub 倉庫

### 3️⃣ 建立 PostgreSQL 服務
- Railway 會自動添加 `DATABASE_URL`

### 4️⃣ 部署完成！
URL 會自動生成（例如 `https://vibejam-production.up.railway.app`）
